...
<div class="publications">
  {% bibliography -f others --template "{% raw %}{{ reference }}{% assign venue = entry.journal | default: entry.booktitle | default: entry.institution | default: entry.school | default: entry.publisher %}{% if venue %} <em>{{ venue }}</em>{% endif %}{% endraw %}" %}
</div>
